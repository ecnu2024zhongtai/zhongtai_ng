...
export class DemandComponent implements OnInit, AfterViewInit {
    ...
    // ];
    public gapSize = 0;
    private baseurl = 'http://law.conetop.cn:8004';
    grid: number[][] = [];
    harbinpp: number[][] = [];
    demandData: number[][] = [];
    myChart: any;
        ...
        this.httpClient.get<any[]>(`${this.baseurl}/api/v1/demands/last10mins`).subscribe(data => {
            ...
            data.forEach(row => {
                if(row && row.lon_index && row.lat_index && row.trip_count){
                    demandGrid[row.lon_index-1][row.lat_index-1] = this.switchCorlor(row.trip_count)
                }
                else{
                    console.log("有问题：",row)
                }
            });
            let echarsData = this.transferEcharsData(demandGrid);let result = this.transferEcharsData(demandGrid);
            this.echarsOption = this.loadOption(echarsData);
    ...
    newGrid(): number[][] {
        return Array.from({ length: 83 }, () => Array(52).fill(5));
    }
